    ...
    async function parametricCalibration(calSamplingPoints) {
        ...
        for (i = 0; i < calSamplingPoints.length; i++)
        {
                ...
                for (j = 0; j < averageTries; j++)
                {
                    ...
                    console.log('Calibration average i='+i+' j='+j +" "+ JSON.stringify(ret));
                    robot.resetPosition();
...
async function testCalibrationPoints()
{
    ...
    const N = 12;  // for example, you can adjust this value
    const width = data.device.w;
    const height = data.device.h;
    const centerX = width / 2;
    const centerY = height / 2;
    const radius = 250;  // you can adjust this value based on the desired size of your circle
    ...
    async function startMultipleCalibration() {
        ...
        var maxi = 6;
        cal.pointArray = [];
        startx = -config.calWidth+7;
        starty = -config.calHeight-1;
        widthx = config.calWidth*2;
        widthy = config.calHeight*2+2;
        x = startx;
        y = starty;
        z = config.defaultHeight;
        calSamplingPoints = [];
