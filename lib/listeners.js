...
async function testCalibrationPoints()
{
    ...
    {   tdata.push(data.pointArray[i]);
    }
    for (let i = 0; i < tdata.length; i++) {
        console.log("Point: "+i+" "+JSON.stringify(tdata[i].screen));
        for (let j = 0; j < tdata.length; j++) {
            console.log("\tPoint: "+j+" "+JSON.stringify(tdata[j].screen));
        }
        await robot.tap(tdata[i].screen.x, tdata[i].screen.y, function() {
