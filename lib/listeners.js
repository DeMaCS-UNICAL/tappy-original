...
exports = module.exports = function(io, robot, config, servo1, servo2, servo3) {
    ...
    function startMultipleCalibration() {
        ...
        retFunction = function(ret)
        {
                ...
                robot.resetPosition();
                if (!ret.status) return;
                cal.pointArray[i][j].position.x = ret.position[0];
                cal.pointArray[i][j].position.y = ret.position[1];
                cal.pointArray[i][j].position.z = ret.position[2];
                cal.pointArray[i][j].screen.x = ret.screen[0];
                cal.pointArray[i][j].screen.y = ret.screen[1];
                ...
                setTimeout(
                    function() {
                        ...
                        x = -config.calWidth + i * config.calWidth*2 / (maxi-1);
                        y = -config.calWidth + j * config.calWidth*2 / (maxj-1);
                        z = config.defaultHeight;
                        touched = false;
