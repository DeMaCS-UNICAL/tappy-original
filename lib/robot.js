...
function Robot(){
    ...
    /*exec('adb shell input swipe 0 0 0 1100',
...
method.tap = async function(screenX,screenY){
    if(!global.headless){
        return method.physical_tap(screenX,screenY);
    }else{
        return method.headless_tap(screenX,screenY);
    }
}
method.swipe = async function(startX, startY, endX, endY){
    if(!global.headless){
        return method.physical_swipe(startX, startY, endX, endY);
    }else{
        return method.headless_swipe(startX, startY, endX, endY);
    }
}
...
{
    this.setCalibrationData(calibration.getDataFromFilePath('calibration.json'));
