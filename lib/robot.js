...
method.getAngles = function() {
    if(this.version == "v2"){
        let angles = this._board.getAllServoPosition();
        return [
            mapNumber(angles[0], this._config.s1.min, this._config.s1.max, this._config.s1.rangeMin, this._config.s1.rangeMax),
            mapNumber(angles[1], this._config.s2.min, this._config.s2.max, this._config.s2.rangeMin, this._config.s2.rangeMax),
            mapNumber(angles[2], this._config.s3.min, this._config.s3.max, this._config.s3.rangeMin, this._config.s3.rangeMax),
        ]
    }
...
method.setAngles = function(t1, t2, t3) {
    ...
    if(this.version == "v2"){
        this._board.setAllServoPosition([t1.toFixed(2), t2.toFixed(2), t3.toFixed(2)]);
    }
    else{
        this._servo1.to(t1);
        this._servo2.to(t2);
...
method.setPosition = function(x, y, z) {
    ...
    console.log("I want to go to effective angles: "+angles)
    ...
    console.log("Converted to raw angles: "+[t1,t2,t3])
