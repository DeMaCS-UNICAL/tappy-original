...
var delay = 400 / steps;
var child_exec;
function Robot(servo1, servo2, servo3, calibrationFile, k, config, version, board = null) {
    ...
    console.log(this._calibrationFile)
...
method.getAngles = async function() {
    if(this.version == "v2"){
        return await this._board.getAllServoPosition();
    }
    return [this._servo1.last.degrees, this._servo2.last.degrees, this._servo3.last.degrees];
};

method.setAngles = async function(t1, t2, t3) {
    ...
    let newpos = this.getPositionForAngles(t1,t2,t3);
    ...
    if(this.version == "v2"){
        this._board.setAllServoPosition([t1, t2, t3]);
    }
    else{
        this._servo1.to(t1);
        this._servo2.to(t2);
    ...
    update.position = newpos; //await this.getPosition();
    io.sockets.emit('update', update); // send updated info to web clients
};

method.getPosition = async function() {
    var angles = await this.getAngles();
...
method.setPosition = function(x, y, z) {
    ...
    var t1 = mapNumber(angles[1], this._config.s1.rangeMin, this._config.s1.rangeMax, this._config.s1.min, this._config.s1.max);
    var t2 = mapNumber(angles[2], this._config.s2.rangeMin, this._config.s2.rangeMax, this._config.s2.min, this._config.s2.max);
    var t3 = mapNumber(angles[3], this._config.s3.rangeMin, this._config.s3.rangeMax, this._config.s3.min, this._config.s3.max);
    
    this.setAngles(t1, t2, t3);
};

method.setSpeed = function(speed) {
    if(this._board != undefined){
        this._board.setServoVelocity(1, speed);
        this._board.setServoVelocity(2, speed);
        this._board.setServoVelocity(3, speed);
    }
}
...
method.getPositionForAngles = function(a1, a2, a3) {
    var t1 = mapNumber(a1, this._config.s1.min, this._config.s1.max, this._config.s1.rangeMin, this._config.s1.rangeMax);
    var t2 = mapNumber(a2, this._config.s2.min, this._config.s2.max, this._config.s2.rangeMin, this._config.s2.rangeMax);
    var t3 = mapNumber(a3, this._config.s3.min, this._config.s3.max, this._config.s3.rangeMin, this._config.s3.rangeMax);
...
method.double_tap = async function(screenX,screenY, delay) {
    if(!global.headless){
            return this.physical_double_tap(screenX,screenY, delay); 
    }else{
        return this.headless_double_tap(screenX,screenY, delay);
    }
}
...
method.physical_tap = async function(screenX, screenY) {
    ...
    this.setPosition(position.x, position.y, this._config.defaultHeight);
    await timeout(this._config.offsetBeforeStartGettingContactZ);
    var touchZ = position.z;
    this.setPosition(position.x, position.y, touchZ * 0.97);
    await timeout(this._config.timeToGoDownBeforeTouch);
    this.setPosition(position.x, position.y, touchZ + this._config.tapOffset);
    await timeout(this._config.timeTouching);
	//console.log("Moving at position "+position.x+" "+position.y+" "+position.z);
    this.setPosition(position.x, position.y, this._config.defaultHeight);
    await timeout(this._config.timeToGoUpAfterTouch);
    return true;
};

method.physical_double_tap = async function(screenX, screenY, duration) {
    this.wantedX = screenX;
    this.wantedY = screenY;
    taps_number = 2; // provato a generalizzarlo, meh 
    console.log("wantedTouch: "+screenX+";"+screenY);
    var position = this.getPositionForScreenCoordinates(screenX, screenY);
    this.setPosition(position.x, position.y, this._config.defaultHeight);
    await timeout(this._config.offsetBeforeStartGettingContactZ);
    ...
    await timeout(this._config.timeToGoDownBeforeTouch);
    for(let it = 0; it < taps_number; it++){
        this.setPosition(position.x, position.y, touchZ + this._config.tapOffset);
        await timeout(this._config.timeTouching *0.6);
        this.setPosition(position.x, position.y, touchZ * 0.9);
        await timeout(this._config.timeToGoUpAfterTouch + duration);
        }
    this.setPosition(position.x, position.y, this._config.defaultHeight);
    await timeout(this._config.timeToGoUpAfterTouch);
    return true;
...
method.headless_double_tap = async function(screenX, screenY, delay) {
    console.log("wantedTouch: "+screenX+"; "+screenY + "; " + delay);
    var to_exec = "adb shell \"input tap "+ screenX+" "+ screenY+ " && sleep " + (delay/1000) + " && input tap "+ screenX+" "+ screenY + "\"";
    child_exec(to_exec,
        function (error, stdout, stderr) {
            console.log('stdout: ' + stdout);
            console.log('stderr: ' + stderr);
...
method.physical_swipe = async function(startX, startY, endX, endY) {
    ...
    this.setPosition(startPosition.x, startPosition.y, this._config.defaultHeight);
    await timeout(this._config.offsetBeforeStartGettingContactZ);
    this.setPosition(startPosition.x, startPosition.y, startPosition.z * 0.97);
    await timeout(this._config.timeToGoDownBeforeTouch);
    console.log("wantedSwipeStart: "+startX+";"+startY);
    wantedX = startX;
    wantedY = startY;
    this.setPosition(startPosition.x, startPosition.y, startPosition.z + this._config.tapOffset);
    await timeout(this._config.timeTouching);
    console.log("wantedSwipeEnd: "+endX+";"+endY);
    wantedX = endX;
    wantedY = endY;
    this.setPosition(endPosition.x, endPosition.y, endPosition.z + this._config.tapOffset);
    await timeout(this._config.timeSwiping + this._config.timeTouching);
    this.setPosition(endPosition.x, endPosition.y, this._config.defaultHeight);
    await timeout(this._config.timeToGoUpAfterTouch);
...
method.go = function(x, y, z, easeType) {
    console.log("Going to: "+x+" "+y+" "+z + " with easeType: "+easeType);
    var pointB = [x, y, z];
